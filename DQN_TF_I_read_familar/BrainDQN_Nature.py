...
class BrainDQN:
	...
	def __init__(self, actions): ## zheng xu recommend good coding style, cf. https://github.com/tensorflow/models/blob/master/im2txt/im2txt/show_and_tell_model.py
		...
		self.W_conv3,self.b_conv3,self.W_fc1,self.b_fc1, self.W_fc2,self.b_fc2 = self.createQNetwork()
	...
	def createQNetwork(self):
		...
		h_conv3_flat = tf.reshape(h_conv3, [-1,3136]) # flat 3d conv result to 1d vector
	...
	def copyTargetQNetwork ( self ):
		self.session.run ( self.copyTargetQNetworkOperation ) # run, the direction is presented in the __init__

	def createTrainingMethod(self):
		self.actionInput = tf.placeholder ( "float", [None, self.actions] ) ##
		self.yInput 	 = tf.placeholder ( "float", [None] ) ##   
		## why multiply Q-value with actionInputs? it to choose
